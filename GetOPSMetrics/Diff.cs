...
/*
 * Add functions: diff_wordMode, diff_wordsToChars, diff_wordsToCharsMunge to count the change number of words
*/
...
namespace DiffMatchPatch
{
    ...
    public class diff_match_patch
    {
        ...
        {
            diff_match_patch diff = new diff_match_patch();
            var wordsResult = diff.diff_wordsToChars(text1, text2);
            var wordText1 = wordsResult[0];
            var wordText2 = wordsResult[1];
            List<string> wordArray = wordsResult[2] as List<string>;
            List<Diff> diffs = diff.diff_main(wordText1.ToString(), wordText2.ToString(), false);
            diff.diff_charsToLines(diffs, wordArray);
            return diffs;
        }
        ...
        protected Object[] diff_wordsToChars(string text1, string text2)
        {
            ...
            // "\x00" is a valid character, but various debuggers don't like it.
            // So we'll insert a junk entry to avoid generating a null character.
            wordArray.Add(string.Empty);
            //wordArray.Add(" ");
            //wordArray.Add("");

            string chars1 = diff_wordsToCharsMunge(text1, wordArray, wordHash);
            string chars2 = diff_wordsToCharsMunge(text2, wordArray, wordHash);
            return new Object[] { chars1, chars2, wordArray };
        }

        private string diff_wordsToCharsMunge(string text, List<string> wordArray,
                                      Dictionary<string, int> wordHash)
        {
            ...
            {
                wordEnd = text.IndexOfAny(delimiters, wordStart);
                if (wordEnd == -1)
                {
                    wordEnd = text.Length;
                }
                ...
                word = text.JavaSubstring(wordStart, wordEnd) + " ";
                wordStart = wordEnd + 1;

                if (wordHash.ContainsKey(word))
                {
                    chars.Append(((char)(int)wordHash[word]));
                }
                else
                {
                    wordArray.Add(word);
                    wordHash.Add(word, wordArray.Count - 1);
                    chars.Append(((char)(wordArray.Count - 1)));
                }
            }
            return chars.ToString();
        }
